...
export class Compile implements ICompile {
  ...
  public async compileAndRun(useCustomArgs: boolean, run: boolean): Promise<boolean> {
    const document = this.getDocument();
    if(document === null) {
      return false;
    }
    await document.save();
    const command = await this.createCompileCommand(document.fileName, useCustomArgs, run)
    if(command === null) {
      return false;
    }
  ...
  private async createCompileCommand(filename: string, useCustomArgs: boolean, run: boolean): Promise<string | null> {
    const command = `& "${this.config.get(Config.CompilerExePath)}" "${filename}"`;
  ...
  private getConfiguredArguments(run: boolean): string[] {
    let configuredArgs: string[] = this.config.get(Config.CompilationArguments) || [];
    if(run) {
