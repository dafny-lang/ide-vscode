...
export function activate(extensionContext: vscode.ExtensionContext) {
    ...
    if (!Capabilities.hasSupportedMonoVersion()) {
        ...
        vscode.window.showErrorMessage(Error.NoSupportedMono, Error.ConfigureMonoExecutable, Error.GetMono)
            .then((selection) => {
                if (selection === Error.GetMono) {
                    vscode.commands.executeCommand("vscode.open", vscode.Uri.parse(Error.GetMonoUri));
                    let restartMessage;
